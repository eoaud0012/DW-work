...
def main():
    ...
    for i, article in enumerate(collected_articles):
        ...
        # 번역 API에는 핵심 내용만 전달하도록 함 (제목, 링크, 날짜 등은 제외)
        translated_title = translate_text(np_title)
        translated_content = translate_text(article_text)
        
        # 최종 메일 본문에 순서 정보는 번역 API 결과와 별도로 직접 결합
        email_body += f"제목: {translated_title} (전체 기사 {total_articles}개 중 {i+1}번 째)\n"
        email_body += f"링크: {url}\n"
        email_body += f"날짜: {article['date']}\n"
