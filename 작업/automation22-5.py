...
def translate_text(text, mode="default"):
    """
    mode="title": 제목 번역  
    mode="content": 기사 본문 전체 번역은 제외하고, 핵심 내용만 간략하게 요약하여 각 항목을 '-' 기호로 시작하도록 작성
    """
    ...
    if mode == "title":
        ...
        # 기사 원문 전체 번역은 제외하고, 핵심 내용만 간략하게 요약하도록 요청합니다.
        user_content = (
            f"다음 텍스트를 참고하여 기사 내용의 핵심만 간략하게 요약해줘. "
            f"전체 번역은 필요 없으며, 각 항목은 '-' 기호로 시작하게 작성해줘:\n\n{text}"
        )
    else:
        user_content = f"다음 텍스트를 한국어로 번역해줘:\n\n{text}"
...
# --------------------------------------------------------------------
# 메인 함수: 기사 수집, 번역/요약 후 이메일 발송 및 터미널 출력(디버깅용)
# --------------------------------------------------------------------
def main():
    keywords = [
        "Beer Market", "Soju Market", "Korean rice wine Market", "Beverage Market", "Bottled Water Company",
        "Carbonated Beverage", "Sparkling Water", "Children Beverage", "Sports Drink", "RTD Coffee", "Engery Drink",
    ...
    for i, article in enumerate(collected_articles):
        ...
        # 제목과 본문 요약 각각 번역 (본문은 전체 번역 없이 핵심 내용만 요약)
        translated_title = translate_text(np_title, mode="title")
        summary = translate_text(article_text, mode="content")
        
        # 신문사 이름 추출 (URL로부터)
        newspaper_name = get_newspaper_name(url)
        
        # 메일 포맷 구성 (기사 원문 전체 번역은 포함하지 않음)
