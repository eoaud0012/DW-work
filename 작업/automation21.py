...
def main():
    ...
    for keyword in keywords:
        ...
        print(f"'{keyword}'에서 {len(articles)}개의 기사 수집됨.")
        if not articles:
    ...
    print(f"총 {len(collected_articles)}개의 기사 처리 예정.")
    ...
    for article in collected_articles:
        ...
        # 추출 실패 시 검색결과의 정보로 대체
        if np_title == "제목 없음":
            np_title = title
        if article_text == "본문 없음":
            article_text = snippet

        full_text = f"제목: {np_title}\n링크: {url}\n날짜: {date}\n\n내용:\n{article_text}"
        translated_text = translate_text(full_text)
        email_body += f"{full_text}\n"
        email_body += f"----- 번역 및 요약 -----\n{translated_text}\n"
        email_body += "=" * 50 + "\n\n"
        print(f"✅ 기사 처리 완료: {np_title}")
